...
//     await apiPatch(
//       `/users/${user._id}/devices/${deviceId}/approve`
//     );
...
//     await apiDelete(
//       `/users/${user._id}/devices/${confirmDeleteId}`
//     );
...
//     // await apiPut(
//     //   `/users/${user._id}/devices/${deviceId}`,
//     //   { isActive }
//     // );
// await apiPut(
//   `/users/${user._id}/devices/${deviceId}`,
...
//     showToast(
//       isActive
//         ? t('devices.toastEnabled')
//         : t('devices.toastDisabled'),
...
//       {/* ⚠️ Confirm Delete Toast */}
//       {confirmDeleteId && (
//         <div
//           className="toast show position-fixed bottom-0 end-0 m-3 text-bg-warning"
//           style={{ zIndex: 10000 }}
//         >
...
const UserDevices = ({ user, isAdmin, onUpdated }) => {
  ...
  const approveDevice = async (deviceId) => {
    try {
      setLoadingId(deviceId);
      await apiPatch(`/users/${user._id}/devices/${deviceId}/approve`);
      showToast(t('devices.toastApproved'));
      onUpdated?.();
    } catch (err) {
  ...
  const deleteDevice = async () => {
    try {
      setLoadingId(confirmDeleteId);
      await apiDelete(`/users/${user._id}/devices/${confirmDeleteId}`);
      showToast(t('devices.toastDeleted'), 'danger');
      onUpdated?.();
    } catch (err) {
      showToast(err?.response?.data?.message || t('common.error'), 'danger');
  ...
  const toggleDevice = async (deviceId, isActive) => {
    try {
      setLoadingId(deviceId);
      await apiPut(`/users/${user._id}/devices/${deviceId}`, { isActive });
      showToast(
